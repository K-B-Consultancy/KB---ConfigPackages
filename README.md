...
All packages are versioned in **lockstep** with the root `package.json` — a change to any package
bumps every `version` field (root + all four packages) in the same PR.
...
3. Publish the release. `publish.yml` verifies the tag matches every `package.json` version (it
   fails the publish otherwise), then publishes all packages to the public npm registry.
